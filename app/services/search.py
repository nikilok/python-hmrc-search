...
def get_fuzzy_score(company_name: str, company: str) -> int:
    """
    Calculate the fuzzy matching score between the search query and a company name
    using token set ratio.
    Args:
...
def search_companies(
    company_name: str, threshold: int = 70
) -> List[CompanySearchResult]:
    """
    Search for companies in the loaded CSV whose names closely match the given
    company_name using fuzzy matching.
    Args:
